...
import { Trash2, Copy, Shuffle } from "lucide-react";
...
export default function AffirmationBuilder() {
  ...
  return (
    <Tabs defaultValue="create">
      <TabsList className="mb-6"><TabsTrigger value="create">Create</TabsTrigger><TabsTrigger value="collection">My Affirmations ({entries.length})</TabsTrigger><TabsTrigger value="daily">Daily Practice</TabsTrigger></TabsList>
          ...
          <div className="grid md:grid-cols-2 gap-3">{entries.map((e: any) => (
            <Card key={e.id} className="glass-card"><CardContent className="pt-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-xs text-accent">{e.entry_data.category} · {e.entry_data.reminder_time}</span>
